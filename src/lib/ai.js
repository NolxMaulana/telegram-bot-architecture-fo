...
async function aiCall(cfg, path, payload = {}, opts = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    const { ctrl, clear } = withTimeout(timeoutMs);
    try {
      if (DEBUG) console.log("[aiCall] ->", url, "attempt", attempt + 1);
...
export async function aiChat(cfg, userText, { system = "You are a helpful assistant.", meta, retries = 1, model } = {}) {
  return aiCall(
    cfg,
    "/chat",
    {
...
async function routeChat(cfg, userText, systemHint) {
  ...
  try {
    ...
    const { text, json } = await safeRead(r);
    const raw = String(json?.text || json?.message || text || "").trim();
    const parsed = json?.route ? json : (() => {
      try {
